...
class SportsSlotsScene: SKScene {
    ...
    private var slots1: SlotMachineBaraban!
    private var slots2: SlotMachineBaraban!
    private var slots3: SlotMachineBaraban!
    ...
    override func didMove(to view: SKView) {
        ...
        let backImage = SKSpriteNode(imageNamed: "sports_slot_bg")
        backImage.size = size
        ...
        let slotsMachineBack = SKSpriteNode(imageNamed: "sport_slots_bg")
        slotsMachineBack.position = CGPoint(x: size.width / 2, y: size.height / 2 + 200)
        slotsMachineBack.size = CGSize(width: size.width - 100, height: 800)
        addChild(slotsMachineBack)
        
        slots1 = SlotMachineBaraban(slotSymbols: slotSymbols, size: CGSize(width: 200, height: 700)) {
                   
        }
        slots1.position = CGPoint(x: size.width / 2 - 280, y: size.height / 2 + 200)

        slots2 = SlotMachineBaraban(slotSymbols: slotSymbols, size: CGSize(width: 200, height: 700)) {
           
        }
        slots2.position = CGPoint(x: size.width / 2, y: size.height / 2 + 200)

        slots3 = SlotMachineBaraban(slotSymbols: slotSymbols, size: CGSize(width: 200, height: 700)) {
            self.checkWinning()
        }
        slots3.position = CGPoint(x: size.width / 2 + 280, y: size.height / 2 + 200)
...
class SlotMachineBaraban: SKSpriteNode {
    ...
    func addSymbols() {
        ...
        for i in 0..<slotSymbols.count * 8 {
            ...
            symbol.size = CGSize(width: 200, height: 200)
            symbol.zPosition = 1
            symbol.name = symbolName
            symbol.position = CGPoint(x: 0, y: size.height - CGFloat(i) * 230.5)
            contentNode.addChild(symbol)
        }
        contentNode.run(SKAction.moveBy(x: 0, y: 230.5 * CGFloat(slotSymbols.count * 3), duration: 0.0))
    }
    
    func startScrolling() {
        if reverseScroll {
            reverseScroll = false
            let actionMove = SKAction.moveBy(x: 0, y: -(230.5 * CGFloat(Int.random(in: 4...6))), duration: 0.5)
